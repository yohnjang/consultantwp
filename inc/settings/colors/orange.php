...
echo '.light-bg,#consultation, body,#primaryMenuWrapper.show,.hamburger-inner, .hamburger-inner::before, .hamburger-inner::after, #cta a.cta.button:hover  , #newsletter_section button:hover, #footer_newsletter button:hover{ background-color: ' . $light_bg_orange . ';}';

echo '.medium-bg,#cta, footer#main #footer_newsletter input[type="submit"],#newsletter_section input[type="submit"],.btn-secondary:hover, #newsletter_section button, #footer_newsletter button { background: '. $medium_bg_orange . '; }';
...
echo '.light-text, #newsletter_section, #consultation_form button, footer#main p, #footer_copyright, #footer_copyright a, #topbar a,#topbar, #cta,#cta a, footer#main a,footer#main, #newsletter_section h2, #static_hero #typed, #static_hero h1, #static_hero .display-2,#newsletter_form_text_logo a, #static_hero p.heading, #newsletter_section .display-3, footer#main #footer_contact .display-5, footer#main .display-4, nav#primary li a, nav#primary .display-3 a, #newsletter_section .newsletter_code p, #newsletter_section p.excerpt,.entry-content .btn-secondary,.btn-secondary  { color: '. $light_text_orange . ';}';

echo '.medium-text, nav#primary ul#primary_menu li:hover a, #topbar .social a:hover, #footer_contact .social a:hover ,ul#footer_menu li a:hover , nav#primary #logo .display-3 a:hover, #footer_text_logo a:hover, #footer_copyright a:hover,#newsletter_form_text_logo a:hover  { color: '. $medium_text_orange . ';}';

echo '.dark-text,#consultation_form label,#footer_newsletter,.display-1,.display-2,.display-3,.display-4,.display-5,.display-6,h1,h2,h3,h4,h5,p, .entry-title a,#newsletter_section input[type="submit"], footer#main #footer_newsletter input[type="submit"], footer#main #footer_newsletter p, #cta a.cta.button:hover , #newsletter_section button, #footer_newsletter button { color: '. $dark_text_orange . ' }';
...
if( get_theme_mod('sparkle_typography_mode_switch') == 1 ){
	...
	#static_hero:before { background:'. $super_dark_bg_orange .';background: linear-gradient(90deg, '. $super_dark_bg_orange .' 0%, '. $super_dark_bg_orange .' 100%, transparent 100%);opacity:1;}

	@media only screen and (min-width:768px){ #static_hero:before {background: '. $super_dark_bg_orange .';background: linear-gradient(90deg, '. $super_dark_bg_orange .' 0%, '. $super_dark_bg_orange .' 100%, transparent 100%);opacity:1;}}

	@media only screen and (min-width:1024px){ #static_hero:before { background: '. $super_dark_bg_orange .';background: linear-gradient(90deg, '. $super_dark_bg_orange .' 0%, '. $super_dark_bg_orange .' 100%, transparent 100%);opacity:1;}}
